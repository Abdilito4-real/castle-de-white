...
const CACHE_NAME   = 'cdwh-v3';
const FONT_CACHE   = 'cdwh-fonts-v2';
const IMAGE_CACHE  = 'cdwh-images-v2';
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .catch(err => {
        ...
        console.warn('[CDWH SW] Pre-cache partial failure:', err);
        self.skipWaiting();
      })
  );
});
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k => !KEEP.includes(k)).map(k => {
          console.log('[CDWH SW] Deleting old cache:', k);
...
<head>
  ...
  <title>Castle de White House Hotel — Offline</title>
  <style>
...
<body>
  ...
  <h1>Castle de White House Hotel</h1>
...
async function openDB() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('cdwh-offline', DB_VERSION);
...
async function syncReservations() {
  console.log('[CDWH SW] Syncing offline reservations…');
  try {
    const config = await getConfig();
    if (!config) { console.warn('[CDWH SW] No Supabase config found, skipping sync'); return; }
    ...
    for (const item of reservations) {
      try {
        ...
        if (res.ok) {
          ...
          console.log('[CDWH SW] Reservation synced:', item.id);
        }
      } catch (err) {
        console.warn('[CDWH SW] Reservation sync item failed:', err);
      }
    }
  } catch (err) {
    console.error('[CDWH SW] syncReservations failed:', err);
  }
}

async function syncNewsletter() {
  console.log('[CDWH SW] Syncing newsletter subscriptions…');
  try {
    const config = await getConfig();
    if (!config) return;
    ...
    for (const item of subs) {
      try {
        ...
        console.warn('[CDWH SW] Newsletter sync item failed:', err);
      }
    }
  } catch (err) {
    console.error('[CDWH SW] syncNewsletter failed:', err);
  }
}
...
self.addEventListener('push', event => {
  ...
  const options = {
    body:     data.body    || 'Your Castle de White House Hotel reservation has been confirmed.',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Castle de White House Hotel', options)
  );
});
...
self.addEventListener('message', event => {
  ...
  if (event.data?.type === 'SET_CONFIG') {
    ...
    console.log('[CDWH SW] Configuration updated');
  }
});
