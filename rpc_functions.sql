...
DECLARE
  ...
  v_ref      TEXT;
BEGIN
  -- Rate Limit: Max 3 bookings per 15 mins per email to prevent abuse
  IF (SELECT COUNT(*) FROM reservations r
      JOIN guests g ON r.guest_id = g.id
      WHERE g.email = p_email
      AND r.created_at > (NOW() - INTERVAL '15 minutes')) >= 3 THEN
    RETURN json_build_object('success', FALSE, 'error',
      'High booking volume detected. Please try again in 15 minutes.');
  END IF;

  SELECT upsert_guest(p_first_name, p_last_name, p_email, p_phone) INTO v_guest_id;

  IF p_room_id IS NOT NULL THEN
    -- Admin is specifying a room, check if it's available for the dates
  ...
  ELSE
    -- Original logic: find any available room of the given type
    SELECT id, room_number, base_price
      INTO v_room_id, v_room_num, v_price
      FROM rooms
...
END;
$$; -- Note: The function signature has changed
GRANT EXECUTE ON FUNCTION create_reservation(TEXT,TEXT,TEXT,TEXT,room_type,DATE,DATE,TEXT,UUID) TO anon, authenticated;


-- ─────────────────────────────────────────────────────────────
-- 5. get_available_rooms_for_type (NEW)
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION get_available_rooms_for_type(
  p_check_in DATE,
  p_check_out DATE,
  p_room_type room_type
)
RETURNS TABLE (id UUID, room_number TEXT)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT r.id, r.room_number::TEXT
  FROM rooms r
  WHERE r.room_type = p_room_type
    AND r.is_active = TRUE
    AND NOT EXISTS (
      SELECT 1 FROM reservations res WHERE res.room_id = r.id AND res.status NOT IN ('cancelled', 'no_show') AND res.check_in_date < p_check_out AND res.check_out_date > p_check_in
    )
  ORDER BY r.room_number;
$$;
GRANT EXECUTE ON FUNCTION get_available_rooms_for_type(DATE, DATE, room_type) TO anon, authenticated;
