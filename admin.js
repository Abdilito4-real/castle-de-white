...
class SupabaseService {
  ...
  async getAvailableRooms(checkIn, checkOut, roomType) {
    ...
    return this.rpc('get_available_rooms_for_type', {
        p_check_in: checkIn,
...
class UIManager {
  ...
  showToast(msg, type = 'info', title = '') {
    ...
    const titles = { success: 'Done', error: 'Error', info: 'Castle de White House Hotel Admin' };
...
class DashboardController {
  constructor(svc, ui) {
    ...
    // Pagination state
    this.pagination = {
      reservations: { page: 1, limit: 12, total: 0, data: [] }
    };
  ...
  _bindGlobals() {
    ...
    window.printInvoice         = (id) => this._printInvoice(id);
    window.roomAction           = (id, num, status) => this._roomAction(id, num, status);
    window.prevPage             = (type) => this._changePage(type, -1);
    window.nextPage             = (type) => this._changePage(type, 1);
    ...
    window.deleteSelectedRooms  = () => this._deleteSelectedRooms();
  ...
  async loadReservations() { /* ... */
    // Fetch all data once (or could be RPC paginated in future)
    const allData = await this.svc.getReservations();
    this.pagination.reservations.data = allData; // Store full dataset
    this.pagination.reservations.total = allData.length;

    this._renderReservations();
  }

  _renderReservations() {
    const { page, limit, data } = this.pagination.reservations;
    ...
    // Update info text
    const elStart = document.getElementById('res-start');
    const elEnd = document.getElementById('res-end');
    const elTotal = document.getElementById('res-total');
    if (elStart) elStart.textContent = data.length > 0 ? start + 1 : 0;
    if (elEnd) elEnd.textContent = Math.min(end, data.length);
    if (elTotal) elTotal.textContent = data.length;

    // Update buttons
    const btnPrev = document.getElementById('res-prev-btn');
    const btnNext = document.getElementById('res-next-btn');
    if (btnPrev) btnPrev.disabled = page <= 1;
    if (btnNext) btnNext.disabled = end >= data.length;

    const container = document.getElementById('reservations-card-grid');
    if (!container) return;

    if (!pageData.length) {
      container.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:3rem;color:rgba(245,239,228,.3);font-size:.8rem">No reservations found</div>`;
      return;
    }
    container.innerHTML = pageData.map(r => {
      ...
      actions.push(`<button class="act-btn" onclick="printInvoice('${r.id}')">Invoice</button>`);

      return `
        <div class="res-card" data-status="${r.status}">
          <div class="res-card-header">
            <div class="res-card-ref">${r.ref_code}</div>
            <span class="${this.ui.pillClass(r.status)}">${r.status.replace(/_/g,' ')}</span>
          </div>
          <div class="res-card-body">
            <div class="res-card-guest">${name}${vip}</div>
            <div class="res-card-room">${room}</div>
            <div class="res-card-dates">${ciDate} &rarr; ${coDate} (${r.nights} nights)</div>
            <div class="res-card-total">${this.ui.fmtMoney(r.total_amount)}</div>
          </div>
          <div class="res-card-footer">${actions.join('')}</div>
        </div>`;
    }).join('');
  }
  ...
  _changePage(type, delta) {
    ...
    if (!state) return;
    state.page += delta;
    if (type === 'reservations') this._renderReservations();
  }
  ...
  async _printInvoice(id) {
    const res = this.pagination.reservations.data.find(r => r.id === id);
    if (!res) { this.ui.showToast('Reservation not found', 'error'); return; }

    const name = res.guests ? `${res.guests.first_name} ${res.guests.last_name}` : 'N/A';
    const roomLabel = res.rooms ? `${res.rooms.room_type.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())} (${res.rooms.room_number})` : 'N/A';
    const ciDate = new Date(res.check_in_date + 'T12:00:00').toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
    const coDate = new Date(res.check_out_date + 'T12:00:00').toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });

    const invoiceHTML = `
      <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Invoice ${res.ref_code}</title>
      <style>
        body { font-family: 'Garamond', 'Times New Roman', serif; margin: 0; padding: 2rem; color: #1a1a1a; background: #fff; }
        .invoice-box { max-width: 800px; margin: auto; padding: 30px; border: 1px solid #eee; box-shadow: 0 0 10px rgba(0, 0, 0, .15); font-size: 16px; line-height: 24px; }
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px; }
        .header .brand h1 { font-family: 'Playfair Display', serif; color: #c8b87a; margin: 0; font-size: 24px; letter-spacing: 2px; }
        .header .brand p { font-size: 14px; color: #555; margin: 5px 0 0; }
        .header .invoice-details { text-align: right; }
        .header .invoice-details h2 { margin: 0; font-size: 28px; color: #333; }
        .header .invoice-details p { margin: 2px 0; font-size: 14px; color: #555; }
        .customer-details { margin-bottom: 40px; }
        .customer-details h3 { margin-bottom: 10px; font-size: 18px; border-bottom: 1px solid #eee; padding-bottom: 5px; }
        ...
        table tr.total td:last-child { border-top: 2px solid #c8b87a; font-weight: bold; font-size: 1.2em; }
        .footer { text-align: center; margin-top: 40px; font-size: 12px; color: #777; }
        .print-btn { display: block; width: 100%; padding: 10px; background: #c8b87a; color: #0e1a0e; border: none; font-size: 16px; cursor: pointer; margin-top: 20px; }
        @media print {
          body { padding: 0; } .invoice-box { box-shadow: none; border: none; margin: 0; padding: 0; } .print-btn { display: none; }
        }
      </style>
      <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500&family=Garamond&display=swap" rel="stylesheet">
      </head><body><div class="invoice-box">
        <div class="header">
          <div class="brand"><h1>CASTLE DE WHITE HOUSE</h1><p>No. 9 Kwato Road, Unguwar Rimi GRA, Kaduna</p><p>reservations@castlewhitehouse.ng</p></div>
          <div class="invoice-details"><h2>INVOICE</h2><p>Ref: ${res.ref_code}</p><p>Issued: ${new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p></div>
        </div>
        <div class="customer-details"><h3>Bill To</h3><p><strong>${name}</strong></p><p>${res.guests?.email || ''}</p><p>${res.guests?.phone || ''}</p></div>
        <table>
          <tr class="heading"><td>Description</td><td>Dates</td><td style="text-align:right">Amount</td></tr>
          <tr class="item">
            <td>${roomLabel}</td>
            <td>${ciDate} - ${coDate} (${res.nights} nights)</td>
            <td style="text-align:right">${this.ui.fmtMoney(res.total_amount)}</td>
          </tr>
          <tr class="total"><td></td><td style="text-align:right"><strong>Total:</strong></td><td style="text-align:right"><strong>${this.ui.fmtMoney(res.total_amount)}</strong></td></tr>
        </table>
        <div class="footer"><p>Thank you for choosing Castle de White House Hotel.</p><p>Payment is due upon arrival. All prices in NGN.</p></div>
    ...
    if (win) {
      ...
      this.ui.showToast('Please allow pop-ups to view the invoice.', 'error');
    }
  }
  async loadRooms() { /* ... */
      ...
      return `<div class="room-tile" data-status="${st}" data-id="${r.id}">
        <div class="room-tile-header">
          <div style="display:flex;align-items:center;gap:0.6rem">
            <input type="checkbox" class="room-select-cb" value="${r.id}" onclick="event.stopPropagation()" style="accent-color:var(--gold);cursor:pointer">
            <div class="room-tile-num" style="margin-bottom:0">${r.room_number}</div>
          </div>
  ...
  _openNewReservation() { /* ... */
    ...
    const ciInput = document.getElementById('adm-ci');
    const coInput = document.getElementById('adm-co');
    const roomTypeInput = document.getElementById('adm-room-type');
    const roomIdInput = document.getElementById('adm-room-id');

    ciInput.value = fmt(today);
    coInput.value = fmt(tomorrow);
    ciInput.min = fmt(today);
    coInput.min = fmt(tomorrow);

    roomIdInput.innerHTML = '<option value="">Select dates and type first</option>';
    ...
    const updateAvailableRooms = async () => {
      ...
      roomIdInput.innerHTML = '<option value="">Loading rooms...</option>';
      roomIdInput.disabled = true;
      const rooms = await this.svc.getAvailableRooms(ci, co, type);
      if (rooms && rooms.length) {
        roomIdInput.innerHTML = rooms.map(r => `<option value="${r.id}">Room ${r.room_number}</option>`).join('');
        roomIdInput.disabled = false;
      } else {
        roomIdInput.innerHTML = '<option value="">No rooms available</option>';
      }
    };
    ciInput.onchange = coInput.onchange = roomTypeInput.onchange = updateAvailableRooms;
  }
  async _submitNewReservation() { /* ... */
    ...
    const phone    = document.getElementById('adm-phone')?.value.trim() || null;
    const roomType = document.getElementById('adm-room-type')?.value;
    const roomId   = document.getElementById('adm-room-id')?.value;
    const ci       = document.getElementById('adm-ci')?.value;
    const co       = document.getElementById('adm-co')?.value;
    if (!first || !last || !email || !roomId || !ci || !co) { this.ui.showToast('Please fill all fields, including a specific room.', 'error', 'Incomplete'); return; }
    const btn = document.querySelector('#newResModal .topbar-btn.primary');
    if (btn) { btn.textContent = 'Creating…'; btn.disabled = true; }
    const result = await this.svc.createReservationAdmin({ firstName: first, lastName: last, email, phone, roomType, roomId, checkIn: ci, checkOut: co });
  ...
  _filterTable(input, tableId, isSelect = false) {
    // If triggered by search input, use its value. If select, find search input value.
    let searchVal = isSelect 
      ? (document.querySelector(`#${tableId}-search`)?.value || '').toLowerCase()
      : input.value.toLowerCase();
      
    // If triggered by select, use its value. If search input, find select value.
    let statusVal = isSelect 
      ? input.value.toLowerCase()
      : (document.querySelector(`#res-table-filter`)?.value || '').toLowerCase();
  ...
  async _deleteSelectedRooms() {
    const checkboxes = document.querySelectorAll('.room-select-cb:checked');
    if (!checkboxes.length) { this.ui.showToast('No rooms selected', 'info'); return; }
    if (!confirm(`Are you sure you want to delete ${checkboxes.length} rooms?`)) return;
    
    let count = 0;
    for (const cb of checkboxes) {
      const res = await this.svc.deleteRoom(cb.value);
      if (res.success) count++;
    }
    this.ui.showToast(`${count} rooms removed`, 'success', 'Batch Delete');
    this.loadRooms(); this.loadStats();
  }
